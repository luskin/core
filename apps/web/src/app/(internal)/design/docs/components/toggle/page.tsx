  ...
  return (
    <PageView>
      ...
      <Column gap={4} className="mt-8">
        ...
        <Paragraph>The current value of the toggle is: {defaultValue ? 'true' : 'false'}</Paragraph>
        <Toggle onToggledChange={setDefaultValue} toggled={defaultValue} />
        <Heading size="md">Custom lables</Heading>
        <Paragraph>
          This example sets the <code className="text-success-11">offLabel</code> to "AM" and the{' '}
          <code className="text-success-11">onLabel</code> to "PM"
        </Paragraph>
        <Toggle onToggledChange={setCustomValue} toggled={customValue} offLabel="AM" onLabel="PM" />
        <ComponentPropsGrid
          params={[
            {
              name: 'toggled',
